...
public class LoadLevelSelect : MonoBehaviour {
	...
	void Start () {
		...
		foreach (Level level in PackPresets.currentPack.levels) {
			...
			float scaleByWidth = pieceRectSize.x / (level.previewPiece.width * squareSize.x) *canvas.scaleFactor * .7f; //The should take up 70% of piece
			float scaleByHeight = pieceRectSize.y / (level.previewPiece.height * squareSize.y) *canvas.scaleFactor * .7f;
